...
const weka_jar_path = "F:\\Dropbox\\Personale\\Facultate\\Proiecte\\github\\ai-algorithms\\node_modules\\node-weka\\bin\\weka.jar";
const tmp_folder = "F:\\tmp\\";
...
(function () {
  ...
  function parseArffFile(arffObj, cb) {
    ...
    async.waterfall([
      ...
      var fileId = tmp_folder + 'node-weka-' + _.random(0, 10000000) + '.arff';
...
//        ' -no-cv -v -p 0');
        ...
        child = exec('java -classpath ' + weka_jar_path + ' ' + options.classifier + ' '
            + options.params +
            ' -t ' + fileIdTraining +
            ' -T ' + fileIdTest +
            ' -no-cv -v -p 0',
